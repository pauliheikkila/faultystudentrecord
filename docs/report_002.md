...
The program gives the default user a default credentials (username and password), which are not changeable. These credentials are extremely weak and easily breakable.

## PoC

This happens everytime the program is run for the first time; it creates a database, creates a default user, which is a weakpoint and is not changeable.

## Impact

This weakness severely weakens the security of this program.

## Remediation

If the default credentials are to be created, the program should immediately ask for a new, secure password. Also, the credentials should be able to be changed at the said users discression.
...
Minun vasemman jalan pikkuvarvas
Anssi Kela

*If you found the vulnerability with a specific tool, you can also credit this tool.*
Sängyn jalka, oikea takanurkka, makuuhuone
Anssi Kelan vasemman jalan isovarvas

## Contact

[info@keskusrikospoliisi.es]
