...
When asked to give a image file, the program doesn't check the file format, which can make the vulnerable for injecting any kind of file into the database.

## PoC

The program doesn't check the file format at any point during the file upload.

## Impact

Possible malicious file can be downloaded into the database, making the database vulnerable to variety of potential misuse and abuse.

## Remediation

Upon upload, the program should check the filetype so it matches the pregiven conditions. Therefore no other files than imagefiles can be uploaded.
